...
module.exports = {
  ...
  registerSlashCommands: () => {
    ...
    (async () => {
      try {
        console.log("-Started refreshing application (slh) commands.\n");

        await rest.put(Routes.applicationGuildCommands(clientId, guildId), {
          body: commands,
        });

        console.log("-Successfully reloaded application (slh) commands.\n");
      } catch (error) {
        console.error(error);
      }
    })();
    ...
    for (const folder of commandFolders) {
      ...
      for (const file of commandFiles) {
        ...
        if (interaction.commandName === command) {
          try {
            require(`../commands/${folder}/${file}`).executeSlash(interaction);
            console.log(
