    ...
    executeSlash: async (interaction) => {
        if (interaction.user.id === "617816411750006794") {
            ...
            try {
              ...
              return await interaction.reply(`Command \`${command.name}\` was reloaded`)
            } catch (error) {
              console.log("(err) " + error)
              return await interaction.reply(`There was an error while reloading\`>${args[0]}\``)
            }
        } else {
            return await interaction.reply("You do not have permission to run that command!")
        }
    },
};
